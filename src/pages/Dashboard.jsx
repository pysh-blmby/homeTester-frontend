...
export default function Dashboard() {
  ...
  const fetchBookings = async (page, limit) => {
    setLoading(true);
    try {
      const res = await api.get(`/bookings/my-bookings?page=${page}&limit=${limit}`);
      if (res.data.success) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 pb-24 px-4 md:px-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {[
          { label: 'Total Items', val: paginationData?.totalItems || 0, icon: Activity, color: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Active Reports', val: bookings.filter(b => b.status === 'Report Uploaded').length, icon: FileText, color: 'text-emerald-600', bg: 'bg-emerald-50' },
          { label: 'Total Pages', val: paginationData?.totalPages || 0, icon: ClipboardList, color: 'text-indigo-600', bg: 'bg-indigo-50' },
          { label: 'Medical History', val: 'Full', icon: ShieldCheck, color: 'text-violet-600', bg: 'bg-violet-50' },
        ].map((stat, i) => (
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <ClipboardList className="w-5 h-5 text-blue-600" /> Your Bookings
          </h2>
          <Button variant="ghost" size="sm" onClick={() => fetchBookings(currentPage, limit)} className="text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg">
