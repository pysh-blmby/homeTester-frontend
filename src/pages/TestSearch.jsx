...
export default function TestSearch() {
  ...
  useEffect(() => {
    fetchTests(keyword, currentPage, limit);
  }, [currentPage, limit]);

  const fetchTests = async (query, page, limit) => {
    setIsLoading(true);
    try {
      const res = await api.get(`/tests/search?keyword=${query}&page=${page}&limit=${limit}`);
      if (res.data.success) {
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {(['All', 'CBC', 'Thyroid', 'Diabetes', 'Vitamin D', 'Lipid Profile'] || []).map(tag => (
            <button 
              key={tag} 
              onClick={() => { 
                const newKeyword = tag === 'All' ? '' : tag;
                setKeyword(newKeyword); 
                onPageChange(1);
                fetchTests(newKeyword, 1, limit); 
              }}
